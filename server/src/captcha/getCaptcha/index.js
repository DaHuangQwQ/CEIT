...
const sendEmail = require('../../common/sendEmail/index').main
...
exports.main = async (event, context) => {
    const {email, subject, content} = event
    let captchaStr = randomString({
        length: 6
    })
    const res = await CaptchaModel.create({
        captcha: captchaStr,
        create_date: new Date().getTime() + 10 * 60 * 1000
    })
    console.log(res)
...
//         console.log(`user unionId = ${unionId} try getCaptcha`);
//         return {
//             result: false,
//         };
//     }
